    ...
    try {
      ...
      const checkRateLimit = (res: Response) => {
        ...
        if (remaining && parseInt(remaining) < 100) {
          ...
          console.warn(`[syncGithub] Low rate limit: ${remaining} requests remaining. Resets at ${resetTime.toISOString()}`);
        }
      };
      ...
      if (parts.length < 2 || !parts[1]) {
        ...
        if (!userRes.ok) {
          if (userRes.status === 403) throw new Error("GitHub API rate limit exceeded. Please wait before syncing again.");
      ...
      checkRateLimit(repoCheckRes);
      if (repoCheckRes.status === 403) throw new Error("GitHub API rate limit exceeded. Please wait before syncing again.");
...
export const createBranch = action({
  ...
  handler: async (ctx, args): Promise<{ branchSessionId: Id<"teamSessions">; branchCustomId: string; groupId: string; groupName: string }> => {
    ...
    // Skip Gemini naming for now - it's causing timeouts
    // Just use simple defaults
    console.log("[createBranch] Using default names (Gemini disabled to prevent timeout)");
    ...
    if (existingGroupId) {
      ...
      // Limit to 500 files max to prevent timeout
      const filesToCopy = mainFiles.slice(0, 500);
      if (mainFiles.length > 500) {
        console.warn(`[createBranch] Too many files (${mainFiles.length}), only copying first 500`);
      }
      ...
      for (let i = 0; i < filesToCopy.length; i += batchSize) {
        ...
        console.log(`[createBranch] Copied ${Math.min(i + batchSize, filesToCopy.length)}/${filesToCopy.length} files`);
      }
