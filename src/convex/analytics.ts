...
type Ids = { site: string; ga4Id?: string; gtmId?: string };

// Shared write path. Empty string is how the admin form clears a field; treat
// it as unset rather than storing a value no tag can use. Validated so a typo
// fails here rather than silently never firing a tag.
async function upsert(ctx: MutationCtx, args: Ids) {
  ...
  return { ok: true, site: args.site, ga4Id: ga4Id ?? null, gtmId: gtmId ?? null };
}
...
export const saveAnalyticsConfig = mutation({
  ...
  handler: async (ctx, args) => {
    requireAdmin(args.adminToken);
    return await upsert(ctx, args);
  },
});

// CI seed path. Internal, so no client can reach it; the only way in is
// `npx convex run` holding the deployment's deploy key — which already has full
// write access to this deployment, since it is what deploys the code. Used by
// .github/workflows/set-analytics-ids.yml so the ids can be written without the
// admin token being typed or pasted anywhere.
export const seedAnalyticsConfig = internalMutation({
  args: {
    site: v.string(),
    ga4Id: v.optional(v.string()),
    gtmId: v.optional(v.string()),
  },
  handler: async (ctx, args) => await upsert(ctx, args),
});
