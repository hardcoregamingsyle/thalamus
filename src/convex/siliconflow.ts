...
// Pollinations.ai is free, keyless, and unlimited. Images are served at a stable
// URL: https://image.pollinations.ai/prompt/{prompt}
// Optional params: width, height, seed, model (flux, turbo)
...
export async function generateImage(
  ...
  const url = `https://image.pollinations.ai/prompt/${encoded}?width=${width}&height=${height}&model=${model}&seed=${Math.floor(Math.random() * 100000)}`;
  return [url];
}
...
export async function generateImageHtml(
  ...
  const alt = prompt.replace(/"/g, "&quot;").slice(0, 200);
  return `<div class="pollinations-image"><img src="${urls[0]}" alt="${alt}" style="max-width:100%;border-radius:8px;margin:8px 0;" loading="lazy" /><p style="font-size:11px;opacity:0.6;margin:2px 0;">Generated by Pollinations AI · <a href="${urls[0]}" target="_blank" rel="noopener">Open full size</a></p></div>`;
}
...
export async function generateVideo(
  ...
  throw new Error("Video generation is not available yet.");
}
