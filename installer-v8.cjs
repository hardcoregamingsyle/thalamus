/**
 * Thalamus Installer v8.0.0  —  NATIVE EDITION
 * Installs the NATIVE C#/.NET 8 (WPF + WebView2) Thalamus app. NO Electron,
 * NO Neutralino, NO bundled browser/Node runtime.
 *
 * Bundles / provisions everything required:
 *   • Thalamus.exe        — native C#/.NET 8 self-contained app
 *   • WebView2 Runtime    — native app UI dependency (installed if missing)
 *   • QEMU                — the VM engine
 *   • thalamus-vm-bridge  — the QEMU bridge (local WebSocket/HTTP control server)
 *   • tvnviewer.exe       — VNC viewer for VM display
 *   • aria2c.exe          — download manager for large OS ISOs
 *   • OS ISOs             — optional, user-selected
 *
...
const PORT = 7891;
const INSTALLER_VERSION = "8.0.0";

// ── URLs ──────────────────────────────────────────────────────────────────────
// NATIVE BUILD ONLY — no Electron, no Neutralino, no bundled browser.
// Thalamus.exe is a native .NET 8 (C#/WPF) single-file, self-contained app that
// renders the Thalamus UI through the OS-native Edge WebView2 control.
...
// Evergreen WebView2 Runtime bootstrapper — required by the native WPF app.
// Most Windows 10/11 machines already have it; we install it only if missing.
...
function getPaths(installDir) {
  return {
    ...
    webview2Setup: path.join(os.tmpdir(), "MicrosoftEdgeWebview2Setup.exe"),
...
// ── Download native C# / WPF app (NOT Electron) ───────────────────────────────
// Thalamus.exe is a self-contained .NET 8 single-file native executable. It uses
// the OS-native Edge WebView2 control to render the UI — there is no bundled
// Chromium/Node runtime of any kind.
function downloadDesktopApp(paths) {
  return new Promise(function(resolve) {
    if (fs.existsSync(paths.desktopExe)) {
      addLog("Thalamus native app already installed.");
      resolve(); return;
    }
    addLog("Downloading Thalamus native app (C#/.NET 8, ~156 MB)...");
...
// ── Install WebView2 Runtime (required by the native WPF app) ──────────────────
// Detects the Evergreen WebView2 runtime in the registry; downloads and silently
// installs the Microsoft bootstrapper only when it is missing.
function isWebView2Installed() {
  // Synchronous best-effort registry check via reg.exe is done in installWebView2.
  return false;
}

function installWebView2(paths) {
  return new Promise(function(resolve) {
    if (process.platform !== "win32") { resolve(); return; }
    // Check registry for the Evergreen runtime GUID.
    var checkCmd = 'reg query "HKLM\\SOFTWARE\\WOW6432Node\\Microsoft\\EdgeUpdate\\Clients\\{F3017226-FE2A-4295-8BDF-00C3A9A7E4C5}" /v pv';
    exec(checkCmd, { windowsHide: true }, function(err, stdout) {
      if (!err && stdout && /\d+\.\d+/.test(stdout)) {
        addLog("WebView2 Runtime already present.");
        resolve(); return;
      }
      // Also check per-user hive.
      var checkCmd2 = 'reg query "HKCU\\SOFTWARE\\Microsoft\\EdgeUpdate\\Clients\\{F3017226-FE2A-4295-8BDF-00C3A9A7E4C5}" /v pv';
      exec(checkCmd2, { windowsHide: true }, function(err2, stdout2) {
        if (!err2 && stdout2 && /\d+\.\d+/.test(stdout2)) {
          addLog("WebView2 Runtime already present (per-user).");
          resolve(); return;
        }
        addLog("Installing WebView2 Runtime (required by native app)...");
        progress.message = "Installing WebView2 Runtime...";
        downloadFile(WEBVIEW2_URL, paths.webview2Setup, function() {}).then(function() {
          exec('"' + paths.webview2Setup + '" /silent /install', { timeout: 180000, windowsHide: true }, function(e) {
            if (e) addLog("WebView2 install note: " + e.message);
            else addLog("WebView2 Runtime installed.");
            resolve();
          });
        }).catch(function(e) {
          addLog("WebView2 download note: " + e.message + " (app may prompt to install it on first launch)");
          resolve();
        });
      });
    });
...
function saveInstallInfo(paths) {
  var info = {
    appType: "native-csharp-wpf",        // NOT electron / neutralino
    ...
    components: ["Thalamus.exe (native)", "thalamus-vm-bridge.exe", "QEMU", "tvnviewer.exe", "aria2c.exe", "WebView2 Runtime"],
  };
...
async function runInstall(selectedISOs, installDir) {
  try {
    ...
    // Step 6: Ensure WebView2 Runtime is present (native app dependency)
    progress.step = "webview2"; progress.message = "Checking WebView2 Runtime..."; progress.percent = 39;
    await installWebView2(paths);

    // Step 7: Download native C# / .NET 8 app (NOT Electron)
    progress.step = "app"; progress.message = "Downloading Thalamus native app (~156 MB)..."; progress.percent = 42;
    await downloadDesktopApp(paths);

    // Step 8: Download VNC viewer
...
var server = http.createServer(function(req, res) {
  ...
  if (req.method === "GET" && req.url === "/api/launch-app") {
    ...
    if (fs.existsSync(paths3.desktopExe)) {
      // Thalamus.exe is the native .NET 8 (C#/WPF) executable — launched directly.
      spawn(paths3.desktopExe, [], { detached: true, stdio: "ignore", cwd: paths3.appDir }).unref();
      addLog("Thalamus native app launched.");
    } else {
      addLog("Thalamus.exe not found — native app may still be downloading.");
    }
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ ok: true }));
    return;
    ...
    <div class="card">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:12px;">
        <div style="display:flex;align-items:center;gap:8px;color:var(--text);">
          <span style="color:var(--success)">✓</span> Thalamus Native App (C#/.NET&nbsp;8)
        </div>
        <div style="display:flex;align-items:center;gap:8px;color:var(--text);">
          <span style="color:var(--success)">✓</span> QEMU VM Engine
        </div>
        <div style="display:flex;align-items:center;gap:8px;color:var(--text);">
          <span style="color:var(--success)">✓</span> WebView2 Runtime
        </div>
        <div style="display:flex;align-items:center;gap:8px;color:var(--text);">
