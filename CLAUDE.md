...
- **`src/convex/lib/`** holds pure helper modules (no Convex framework imports):
  ...
  - Provider clients: `ollamaClient.ts` (formerly `siliconflow.ts` — export names unchanged), `zenClient.ts`, `deadlySignalsClient.ts`, `modelscopeClient.ts`, `modalClient.ts`.
...
- Provider chain when a `ctx` is passed: **Modal** (admin-registered `modalEndpoints`, primary row first) → **OpenCode Zen** (anonymous free tier, optional `ZEN_API_KEY`) → **DeadlySignal** (keyed, `DEADLYSIGNALS_API_KEY`) → **ModelScope** (keyed, `MODELSCOPE_API_KEY`, `.ai` host — `.cn` host rejects tokens) → **Ollama Cloud** (keyed). Without a `ctx` the chain still runs from Zen onward.
- Dispatcher short-circuits: if `assignedModel` matches `findZenModel` / `findDeadlySignalsModel` / `findModelScopeModel`, that provider is tried first and only falls back to the full chain if it errors — Modal is skipped for that call because it does not know those catalog ids.
- `callModel` returns a provider-tagged tier string: `modal:<model>`, `zen:<model>`, `deadlysignals:<model>`, `modelscope:<model>`, `ollama:<model>`. `calcAgentBucksForTier` branches on the prefix; the three keyless/free-tier prefixes cost 0, Modal delegates to `calcModalAgentBucks`, Ollama delegates to `calcAgentBucksForModel`.
...
- **Token resolution is live, not snapshotted.** `githubConfigs.githubToken` is written once when a branch's repo is created. Reading it directly meant a user who reconnected GitHub kept dispatching with the *old* token forever, so a branch could sit permanently on "reconnect GitHub" that reconnecting could never clear. Every runner path goes through `resolveTokenForBranch` (branch owner's current `users.githubAccessToken`, but only when that account actually owns `cfg.owner` — a branch whose repo was auto-created under the platform's own `GITHUB_TOKEN` must keep using the snapshot, since the user's token has no access to it → stored snapshot → `GITHUB_TOKEN`).
