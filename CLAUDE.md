...
| Variable | Consumer |
|---|---|
| `KIMI_API_KEY` | Kimi for Coding (`lib/kimiClient.ts`) — Moonshot coding-plan endpoint, `kimi-for-coding` alias; first keyed chain leg, between Modal and Zen |
...
`functions/geo.js` returns the visitor's country from Cloudflare's edge
(`request.cf.country`) so the analytics consent banner can be shown to UK/EU/EEA
visitors only — see the Analytics section below. It is `no-store` on purpose: a
shared cache entry would hand one country's privacy regime to the next visitor.
...
- `src/pages/` — top-level routes. Several are shells over subdirectories:
  - `Portal.tsx` dispatches to `pages/portal/` (GuestPortal, PortalDesktop, ModeSelection) with `modes.ts`, `guestSession.ts`, `types.ts`, `suggestions.ts`. Auth is checked before the mobile split, so guest mode works on mobile.
    **Study is the only mode.** `VALID_MODES`/`ALL_MODES` in `modes.ts` hold `study` alone; chat, research, code and the six writing/strategy variants were removed because the orchestrator now reads a request and either answers it or plans it, so classifying it up front was work the user should never have been doing. The `Mode` TYPE keeps all ten literals on purpose — `conversations.mode` has rows written under every one of them, and narrowing it would make that legacy data unreadable. `ModeSelection` offers two entries: Agent (→ `/portal/code`) and Study. An unknown `/portal/<mode>` resolves `activeMode` to null and falls back to the picker, so old links land somewhere sensible rather than 404. **`src/content/blog.ts` still links to and describes `/portal/research` in six places** — those URLs soft-land on the picker, but the copy now describes a mode that does not exist; it is live SEO content and was left for a deliberate content pass.
...
- Provider chain when a `ctx` is passed: **Modal** (admin-registered `modalEndpoints`, primary row first) → **Kimi for Coding** (keyed, `KIMI_API_KEY`; `kimi-for-coding` — a paid coding plan the owner already pays for, seated ahead of every free tier so Coder-class seats use it; skipped fast when unconfigured) → **OpenCode Zen** (anonymous free tier, optional `ZEN_API_KEY`) → **OrcaRouter** (keyed, `ORCAROUTER_API_KEY`; `qwen/qwen3.8-27b-free`, skipped fast when unconfigured) → **OpenRouter** (keyed, `OPENROUTER_API_KEY`; defaults to the `openrouter/free` auto-router because the `:free` roster rotates) → **DeadlySignal** (keyed, `DEADLYSIGNALS_API_KEY`) → **ModelScope** (keyed, `MODELSCOPE_API_KEY`, `.ai` host — `.cn` host rejects tokens) → **HuggingFace** (keyed, `HF_TOKEN`; Inference Providers router — `Qwen/Qwen3.8-2.4T-A95B` and 100+ open models, thin free monthly credit, skipped fast when unconfigured) → **Ollama Cloud** (keyed). Without a `ctx` the chain still runs from Kimi onward.
- Explicit-seat short-circuits: if an explicit `assignedModel` override matches `findKimiModel` / `findZenModel` / `findOrcaRouterModel` / `findOpenRouterModel` / `findDeadlySignalsModel` / `findModelScopeModel` / `findHuggingFaceModel`, that provider is tried first and only falls back to the full chain if it errors — Modal is skipped for that call because it does not know those catalog ids. (The pipeline passes no assignments since the Dispatcher was removed — the path exists for overrides.)
...
`dispatchReady`. **`planRun` triages first**: one call decides whether the
request is a question or a project. A question is answered there and then —
the run completes with the answer in `codeRuns.summary` and a `run.answered`
event, and no task is ever created. Only a project is decomposed. The two
exits are told apart by the output itself (a fenced JSON plan means build it,
prose means it is already answered), which is why the prompt forbids the answer
branch from emitting JSON. **Planning is the only sequential step**, which makes it the
...
whose they are, because several tasks can have commands in flight on one branch
at once. **The executors needed no change** — both the GitHub Actions worker
(`claimPendingCommandsForVm`) and the desktop local executor
(`listPendingForBranch`) claim by BRANCH, so a parallel run's commands are
picked up as-is. The budgets are wall-clock, not turn counts, because the whole
loop lives in one action and Convex's 10-minute kill is the real ceiling:
`TASK_WALL_CLOCK_MS` 8 min, `COMMAND_WAIT_MS` 3 min, `MIN_TURN_MS` 45 s. A task
always finishes deliberately and writes down what it did — being killed mid-turn
would leave the row in `running` with `finishTask` never coming. A command that
nobody picks up is reported to the agent as unfinished, never as passing.

Still missing: MCP tool calls stay legacy-pipeline only, and
`codeTasks.parentTaskId` (a task tree) is unimplemented. **There is no desktop
counterpart to the Runs tab yet** — the WPF app still drives the legacy
pipeline only.
...
IDs live in the `analyticsConfig` table (one row per product, `/admin` →
Analytics), not in build-time env vars: both frontends read the same backend, so
a property change would otherwise mean rebuilding and redeploying two Cloudflare
Pages projects. `analytics.getAnalyticsConfig` is public and returns nulls when
unset, which is what keeps the tags off until someone configures them.

`components/Analytics.tsx` (mirrored in the AgentOverflow repo) decides by
region, not by a global default:

- **outside the UK/EU/EEA** — tags load on first paint, nothing suppressed.
- **inside it** — nothing loads until the visitor accepts the banner.

That is the reason there is no blanket IP-anonymisation call anywhere: in the
consent regions no hit fires at all, which is stronger than anonymising one that
already went out; outside them the product decision is untouched analytics. The
region comes from `functions/geo.js` and fails closed — an unknown country is
treated as needing consent. `tests/geoConsent.test.ts` pins the country set.

The ids can also be written without the admin token: `.github/workflows/
set-analytics-ids.yml` is a dispatch-only workflow that calls the internal
`analytics.seedAnalyticsConfig` with the repo's `CONVEX_DEPLOY_KEY` — the same
credential that deploys the backend, so it grants nothing new.

GA4 and GTM are both supported because they are not alternatives. Configuring a
GA4 tag *inside* GTM while also setting a GA4 ID counts every pageview twice —
set one or the other.

Web only. The WPF desktop app ships no web analytics and has no counterpart to
add one to.
...
- **Corpus search is switched off at the VM and the Qdrant container is gone.** Search was the only thing needing the vector index, the index was the only thing needing a big machine (2.45M x 384-d vectors + HNSW, 5g cap against 2g each for Postgres and the API), and no agent was calling `/v1/search`. The `/q` pages carrying all ~200k monthly impressions are a Postgres row lookup and never touched it, so the corpus stays fully served. The VM answers 503 for every search path; `backendDownResult` in `agentoverflowHttp.ts` maps that to code `search_disabled` — NOT `backend_unavailable` — because a deliberate state must not read as an outage and send someone hunting a healthy VM. Credits are still refunded on that path. Re-enabling is a VM-side change only (restore the compose service, clear `AO_SEARCH_DISABLED`); nothing here needs editing.
