...
* **`agentCore.ts`:** the model brain — the `FREE_UNLIMITED` switch, `callModel` (the one router), `mapModelIdToOllama`, `calcAgentBucksForTier`, `performSearch`/`performScrape`, the JSON-op output parser (single-line `{"op":…}` ops; legacy `<<TAG>>` markers still parsed as fallback for old stored messages), and every agent system prompt (treat prompt edits like schema migrations). It re-exports the actual provider clients from `siliconflow.ts` (Ollama Cloud — the filename is a leftover, its header and base URL say Ollama), `nimClient.ts` and `modalClient.ts`. No Bedrock code lives here any more; the SigV4 signers are in `http.ts`, `ai.ts` and `study.ts`.
...
* Task type → NIM model: dispatcher `meta/llama-3.1-8b-instruct` (fast routing seat, verified healthy; `meta/llama-3.2-3b-instruct` started hanging on NVIDIA's side and `nemotron-mini-4b` is served at a 4096-token context cap that trips the 8192 max_tokens request), everything else in the pipeline — code, reasoning, agent, factcheck, research — `deepseek-ai/deepseek-v4-flash`, else `NIM_DEFAULT_CHAT_MODEL`. Ollama's equivalent is `mapModelIdToOllama` in `agentCore.ts` (gemma4:31b / minimax-m3 / gpt-oss:120b). The Dispatcher call carries a 60s fail-fast deadline so a dead/hung provider surfaces in about a minute instead of burning the whole chain budget. NIM retries ride out NVIDIA's 529-overload bursts: up to 3 keys × 3 rounds, bounded by the same deadline.
