...
Behaviors worth knowing: MCP tool calls (user-connected servers plus the built-in AgentOverflow and Sketchfab servers, bounded rounds), `<<RUN-CMD>>` execution (GitHub Actions in the cloud, the user's own machine on desktop), `<<REQUEST-API-KEY>>` pause/resume, Critic retry loop (max 2), simulated streaming (the batch response is drip-fed to `streamingContent` in 300-char chunks — real token streaming was abandoned as unreliable in Convex actions).
