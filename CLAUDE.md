...
- Provider chain when a `ctx` is passed: **Modal** (admin-registered `modalEndpoints`, primary row first) → **OpenCode Zen** (anonymous free tier, optional `ZEN_API_KEY`) → **OpenRouter** (keyed, `OPENROUTER_API_KEY`; defaults to the `openrouter/free` auto-router because the `:free` roster rotates) → **DeadlySignal** (keyed, `DEADLYSIGNALS_API_KEY`) → **ModelScope** (keyed, `MODELSCOPE_API_KEY`, `.ai` host — `.cn` host rejects tokens) → **Ollama Cloud** (keyed). Without a `ctx` the chain still runs from Zen onward.
...
`codeProjects` → `codeBranches` → `codeMessages` / `codeFiles` / `codeCommands` / `codeApiKeys*` / `mcpServers`. Driven by `src/convex/codePipeline.ts` at `/portal/code/*` in `CodeProjects` → `CodeBranches` → `CodeWorkspace`. A Dispatcher runs before EVERY pipeline — a fresh user prompt always re-enters through it (interrupting any in-flight run) and it re-runs between tasks — and persists the picked agents on `codeBranches.dispatchedAgentsJson`, per-agent model assignments on `dispatchedModelsJson`, custom agents on `customAgentsJson`, and the first-iteration skip list on `skipAgentsJson`/`skipActive`. `userPromptGen` is bumped per prompt; all phase transitions go through the `advance()` helper, which refuses when the generation moved so a newer prompt's dispatch can never be clobbered by a superseded chain.
...
- **Token resolution is live, not snapshotted — and provenance-aware.** `githubConfigs.githubToken` is written once when a branch's repo is created. Reading it directly meant a user who reconnected GitHub kept dispatching with the *old* token forever, so a branch could sit permanently on "reconnect GitHub" that reconnecting could never clear. Every runner path goes through `resolveTokenForBranch`, which returns `{token, source}`: the branch owner's current `users.githubAccessToken` only when that account actually owns `cfg.owner` (`source: "user"`), otherwise the stored snapshot/`GITHUB_TOKEN` (`source: "platform"` — the user token has no access there at all). Callers must check `source` before printing reconnect advice: on a `"platform"` token failure, reconnecting the user's GitHub can never change anything, and the runner says exactly that (`PLATFORM_WORKFLOW_SCOPE_MSG`) instead of looping the user through OAuth — the user-loop version of this bug is exactly the "I keep reconnecting and nothing changes" report.
- **`executorBlockedReason` is only set on a *proven* block, and its wording is ownership-aware.** A 403/404 writing under `.github/workflows/` is put to GitHub via `x-oauth-scopes` before it is called a scope problem; a missing branch ref is created and retried; anything else stays retryable and sets nothing. `startPipeline` awaits `bootVmForBranch` so the warning it prints reflects this prompt, not a stale one. When the token genuinely lacks `workflow`, the stamped reason splits by `resolveTokenForBranch`'s `source`: `"user"` gets the reconnect instruction (true, and one-time — every branch on the account heals at once), `"platform"` gets the admin-facing truth (platform token needs the scope; the desktop local executor is the user's working path). The Git Sync banner makes the same distinction by comparing the repo `owner` to the connected username.
- **`workflow_dispatch` requires the workflow file on the repo's DEFAULT branch, not just the working branch.** GitHub only registers the trigger via the default branch; a copy that exists solely on the per-branch working repo's own branch makes `createWorkflowDispatch` 404 unconditionally, forever — confirmed by dispatching against a live throwaway branch/workflow on this repo. `ensureWorkflowOnRepo` (`githubActionsRunner.ts`) writes the workflow to both the default branch (to register it) and the working branch (which is what actually executes — the run's `head_branch` matches the working branch, not main). This was the real cause of every prior "workflow-scope-missing" / VM-boot failure on a freshly created branch; it had nothing to do with OAuth scopes. Do not revert to writing only `cfg.branch`.
