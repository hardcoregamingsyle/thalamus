...
### Blocked executor, warned once

A proven block stamps `codeBranches.executorBlockedReason` (ownership-aware wording: platform-side failures name the platform, user-token failures name the missing scope). `startPipeline` then surfaces a ⚠️ "Cloud command execution is disabled" System message — **once per distinct reason, never once per prompt**. The branch's `executorBlockWarnedReason` stamp remembers which wording was already shown; `setExecutorBlocked` clears the stamp whenever the reason itself changes (a heal clears it too), so a persistent block goes quiet after its first announcement while a genuinely new situation is still announced exactly once. (The earlier dedupe asked only "is the last transcript message this warning?" — the user's own "continue" is a User row, so every prompt re-armed the identical warning.) The blocked prompt also strips the `cmd` op advertisement and forbids `request-api-key` for GitHub credentials, because no user-pasted token can clear a platform-side block. The decision logic lives in the pure, unit-tested `src/convex/lib/executorWarnings.ts`.
