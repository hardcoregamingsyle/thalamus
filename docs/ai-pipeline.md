...
From there the run is agent-routed end to end: each agent ends its turn naming the next teammate with `{"op":"over-to"}`, and the run ends when the Critic accepts the final task, the Analyser names nobody, or KnowItAll finishes a plain answer. A reply that ends silent on routing — or aims at a name that is not a teammate — routes NOTHING: the speaker is coached in its own transcript line and runs again under the same `MAX_CONTINUE_ROUNDS` bound, so movement is always a decision an agent made, never something the system inferred. The agents control the pipeline — the system only carries the plan forward on acceptance and enforces nothing else.
...
|---|---|---|
| Analyser | `reasoning` | The lead. Opens every run, analyses, and directs the team with over-to. Steps in only when a seat has refused the ending contract through the whole coached bound; closes the run itself with `{"op":"done"}` when nothing remains to delegate. |
...
6. **No/invalid hand-off** — falls back to the Analyser… except an explicit `{"op":"done"}` from the Analyser or KnowItAll (the designed run end), and the relay/pass exits.

Every route is announced in the transcript — hand-offs are never silent: a `⇄ From → To — why` System line for normal hand-offs (the upgrade note is folded in when a lone research member was named), a `[CONTINUING: no hand-off named — …]` stamp appended to the speaker's own message when it owes the turn an ending (the coach-and-re-run), a `[ROUTING] … Analyser takes over routing` line naming the exact refusal when a coached seat spends its whole bound, and a `✔ Run complete — …` line for the two natural exits. The `[OVER TO: target — why]` marker from the parser also remains inline in the agent's own message, with the why in FULL (it is the receiver's briefing — the old 120/140-char display cuts sliced it mid-word and hid the actual instruction).
...
3. **Kimi for Coding** — Moonshot's keyed coding-plan endpoint (`KIMI_API_KEY`, `api.kimi.com/coding/v1`). `kimi-for-coding` — the documented alias for the plan's current K2-class model (not verified live; treat as perishable). Seated first among the keyed legs, ahead of the anonymous Zen tier, because it is a paid plan the owner already pays for and wants used for Coder-class seats. Same SSE streaming/idle-timeout/salvage shape as the OrcaRouter leg. Skipped fast when the key is unset. `KIMI_DISPATCHER_MODEL` / `KIMI_DEFAULT_MODEL` (`src/convex/lib/kimiClient.ts`).
4. **OpenCode Zen** — anonymous free tier, `ZEN_API_KEY` optional. `ZEN_DISPATCHER_MODEL` / `ZEN_DEFAULT_MODEL` for the two seats (`src/convex/lib/zenClient.ts`).
5. **OrcaRouter** — keyed OpenAI-compatible gateway (`ORCAROUTER_API_KEY`, `api.orcarouter.ai/v1`). `qwen/qwen3.8-27b-free` — strong reasoning-class coding seat, free at this gateway. Same SSE streaming/idle-timeout/salvage shape as the OpenRouter leg. Skipped fast when the key is unset. `ORCAROUTER_DISPATCHER_MODEL` / `ORCAROUTER_DEFAULT_MODEL` (`src/convex/lib/orcaRouterClient.ts`).
6. **OpenRouter** — keyed free-model gateway (`OPENROUTER_API_KEY`, `openrouter.ai/api/v1`). Defaults to the `openrouter/free` auto-router because the `:free` roster rotates (DeepSeek/Gemini/Mistral free variants were pulled in 2026); 20 req/min per free model. Streams via SSE — timeouts are idle-based (60s to the first chunk, 60s between chunks) so a slow-but-alive model keeps the leg until the chain deadline instead of dying on a fixed per-attempt cap; deltas pipe live into `streamingContent`. `OPENROUTER_DISPATCHER_MODEL` / `OPENROUTER_DEFAULT_MODEL` (`src/convex/lib/openrouterClient.ts`).
7. **DeadlySignal** — keyed New API gateway (`DEADLYSIGNALS_API_KEY`, `myapi.creitingameplays.com/v1`). `DEADLYSIGNALS_DISPATCHER_MODEL` / `DEADLYSIGNALS_DEFAULT_MODEL`.
8. **ModelScope** — Alibaba's official free API-Inference tier (`MODELSCOPE_API_KEY` plus fallback pool `MODELSCOPE_API_KEY_2` … `_10`, tried in order on rate-limit/quota/revoked key; `api-inference.modelscope.ai/v1` — the `.cn` host rejects `ms-…` tokens). `MODELSCOPE_DISPATCHER_MODEL` / `MODELSCOPE_DEFAULT_MODEL`.
9. **HuggingFace** — the Inference Providers router (`HF_TOKEN`, `router.huggingface.co/v1`). One free HF token reaches 100+ open-weight models through a single OpenAI-compatible endpoint — including `Qwen/Qwen3.8-2.4T-A95B` (the Qwen 3.8 Max-class open checkpoint, which no other seat in this chain serves; `Qwen/Qwen3.8-27B` itself is NOT on the router yet — verified 2026-08-24 — the OrcaRouter leg covers that model). Seated low because the included free monthly credit is thin (~100K credits, a handful of pipeline calls): when the credit is spent the router answers 402 and the chain falls through. Verified against the router's public `/v1/models` listing, which currently marks every big-model backend `is_free: false` — there is no un-keyed public pool (the `hf.space` "public pool" snippet circulating online is bogus; that domain hosts Spaces apps, not an inference API). Same SSE streaming/idle-timeout/salvage shape as the OpenRouter leg. Skipped fast when the key is unset. `HUGGINGFACE_DISPATCHER_MODEL` / `HUGGINGFACE_DEFAULT_MODEL` (`src/convex/lib/huggingFaceClient.ts`).
10. **Ollama Cloud** — keyed pool (`OLLAMA_API_KEY`, `OLLAMA_API_KEY_2` … `_10`; the module builds the pool dynamically so a literal grep misses those). Task-type mapped by `mapModelIdToOllama()` in `agentCore.ts`.

Without a `ctx`, Modal is skipped and the chain runs from Kimi onward. Every leg wraps its call in try/catch and logs the failure before falling through.
...
**`record` also folds every attempt into `providerHealth`, the chain's platform-wide memory.** A failure is classified by `lib/providerCooldowns.ts` (pure, unit-tested — 6h for auth/balance/model-unavailable, until the next UTC midnight for a daily quota, 3 min for a burst 429, 90s for transient/empty-output, nothing at all for "key not configured"), a success deletes the seat's row, and `callModel` consults `providerLog:liveInternal` once per invocation so a seat that provably cannot serve is **skipped** (with a `SKIPPED — learned <class> cooldown` row via `providerLog:logOnly`) instead of re-attempted on every turn — before this, a dead seat like a `403 vip group` or `402 no balance` was retried three-plus times per turn for 23+ hours. Two invariants: skip notes never pass through `record` (its classifier would re-stamp the very cooldown the note describes), and if EVERY sync seat is cooled the learnings are ignored for that invocation — a cooldown must never empty the chain. The fetch is once per `callModel` invocation, not per chain pass, so the 25s burst-retry pass is never neutered by cooldowns its own first pass just stamped.
...
|---|---|---|
| Kimi | `KIMI_DISPATCHER_MODEL` in `lib/kimiClient.ts` | `KIMI_DEFAULT_MODEL` (both `kimi-for-coding`) |
...
The named agent runs next — the only routing mechanism the run has (see "Team hand-offs" above). `to` is accepted as an alias of `agent`, `reason` as an alias of `why`, and the op-name variants models write (`over_to`, `handover`, `hand-off`, `handoff`) all parse the same way. `"ResearchTeam"` (or any single research member, upgraded to the whole team) starts the four-member Research Team in its fixed order. The pipeline validates the name: an invalid target is NOT routed anywhere — the speaker is coached in its own transcript line and re-runs to name a real teammate (see "No hand-off / bad name" below), and a SELF target (the agent naming itself — recognised whenever the parser is given the speaker's name, which the pipeline always passes) is recorded as `selfHandoffWhy` instead and treated as an implicit continue (see below).

**Marker-echo recovery.** Agents read this same transcript and sometimes re-TYPE the pipeline's square-bracket stamp (`[OVER TO: …]`, `[CONTINUE]`, `[CONTINUING: …]`) instead of the JSON op — the stamp/op distinction is genuinely confusing to a model, and the prompts now say so explicitly ("typing a stamp is not the command"), but the parser no longer depends on the distinction: a stamp sitting at the very END of a reply (nothing but closers/whitespace after it) is honoured as the op it echoes. Mid-prose stamps are quoted examples and never fire, our own `[OVER TO: invalid — …]` stamp recovers to nothing, and a real op — which already set the field — always wins over a trailing echo.
...
A SELF over-to (`{"op":"over-to","agent":"<itself>","why":"what it does next"}`) shares this leg: the parser records it as `selfHandoffWhy`, stamps `[CONTINUING: why]`, and the pipeline re-runs the agent under the same `MAX_CONTINUE_ROUNDS` bound. An end-of-reply `[CONTINUE]`/`[CONTINUING: …]` stamp typed as plain text recovers the same way (see marker-echo recovery above). Two carve-outs: it never fires inside the Research Team (the four-hand relay owns the members' order — a member lingering on its own seat would stall the hand to the next member), and when the cap refuses it the fallback `[ROUTING]` line says so honestly ("kept handing the next step to itself — after 10 rounds of solo work the Analyser takes over routing.") instead of the generic "named no next teammate".

**No hand-off / bad name (the ending contract).** Every reply by a non-terminal seat outside the relay owes ONE ending — `continue` or a real over-to. The contract is decided by ONE pure function, `classifyTurnEnding` in `src/convex/lib/turnContract.ts` (unit-tested in `tests/turnContract.test.ts` end to end); the pipeline merely executes it. A reply that ends silent on routing (**undirected**: no routing intent at all), or one whose over-to names something that is not a teammate (**badTarget**), used to fall back to "the Analyser takes over routing" — which put the *system* in the driver's seat the dispatcher was deleted from. Now the speaker keeps the floor: the breach is stamped into the speaker's own message as `[CONTINUING: no hand-off named — keep working or name the next teammate]` / `[CONTINUING: "<name>" is not a teammate — name a real one, or continue]` and the same agent re-runs under the same `MAX_CONTINUE_ROUNDS` bound (the stamp rides the agent message because the prompt digest drops System rows, so coaching anywhere else would never reach the model it corrects). The exclusions are the designed exits, not loopholes — and every one is STATED, never inferred from silence: the Analyser and KnowItAll close runs with an explicit `{"op":"done","why":…}` (a bare ending from them is the same coached breach as everyone else's — the regression was an Analyser that ended an architecture blueprint with a whole "NEXT STEPS & HANDOFF" section and no op, and the run silently completed while the System line claimed "nothing more to delegate"; a closing seat that spends its whole coached budget still ends, but with the honest "spent N coached turns without delegating or closing" line); a Critic pass IS the decision; the relay owns its members' order (FactCheck's silence hands the findings to the lead). A `done` op from any non-closing seat is ignored — build seats cannot close runs. Only a seat that refuses through the whole bound escalates to the Analyser, with the `[ROUTING]` line naming the exact refusal — held the floor N turns / naming a non-teammate / self-handing / continue past the cap — the last "system routes" moment left, and it is loud, terminal for that seat. **The bound is per-seat (`floorCapForSeat` in `lib/turnContract.ts`):** the Coder's job is legitimately dozens of single-file turns, so its window is **75 turns** and spending it is NOT a takeover — the contract asks a checkpoint question (`[CHECKPOINT] …`, stamped into the Coder's own message so it survives the digest, plus a dedicated next-turn prompt block via `codeBranches.checkpointPending`) that the Coder must answer with VISIBLE step-by-step thinking — what the task required, what exists (files named), what remains — and a stated verdict: over-to the finished work on, or `{"op":"continue"}` which opens a fresh 75-turn window. Only a checkpoint that comes back with no verdict at all escalates (`checkpoint-ignored`). Every other seat keeps the 10-turn bound and the classic takeover, earned ten coached turns.

**Every seat is taught the ending, the Planner included.** The Planner's turn ends with the JSON plan followed by a routing op on its own line (usually `over-to` the Analyser); `parsePlannerOutput`'s backwards brace-walk ignores the trailing op. Its system prompt used to mandate "output ONLY valid JSON" — a Planner that could never comply ended silent by design, and the coaching loop re-ran it nine times in one round, re-rendering a slightly different plan each time while `plannerTasksJson` flapped with every re-call. That is the exact failure teaching the contract at its own seat fixes; the generic hand-off block also states the 10-turn solo limit so the checkpoint escalation never arrives as a surprise.
