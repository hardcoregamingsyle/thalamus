...
A native Windows app that puts four AI tools and a full VM sandbox in one window. WPF on .NET 8. Ships as a single self-contained `.exe` — nothing to install alongside it, no runtime required.
...
There is also a notify-only update check: the app watches GitHub Releases and shows a label when a newer version is available (it never downloads or installs anything itself).
...
Everything publishes self-contained and single-file, so the shipped binary has zero runtime dependencies. The project builds with 0 warnings and 0 errors; keep it that way when contributing.
